...
DESTINATION_BRANCH="chart-release"
BUILD_BASE_DIR="${SCRIPT_DIR}/../helm-chart/build"
...
cd "${BUILD_SOURCE_DIR}/helm-chart"

set +e
if  ! grep -q "version: ${REQUESTED_TAG}" "${BUILD_SOURCE_DIR}/helm-chart/os2display/Chart.yaml" ; then
...
mkdir -p helm-chart
mv "${BUILD_SOURCE_DIR}/helm-chart/${PACKAGE}" helm-chart/
git add "helm-chart/${PACKAGE}"
